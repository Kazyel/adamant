...
function applyDeltas(source: string, deltas: readonly RawDelta[], inverse: boolean): string {
  ...
  for (
    let index = inverse ? deltas.length - 1 : 0;
...
export class MarkdownSourceHistory {
  ...
  apply(
    ...
    if (direction === 'edit') {
      ...
      if (alternativeVersionId === this.version) {
        return this.source;
      }
      let target = this.cursor;
      if (direction === 'undo') {
        while (target > 0 && this.entries[target - 1].before !== alternativeVersionId) {
          target--;
        }
        if (target === 0) {
          return null;
        ...
        while (
          ...
          this.entries[target].after !== alternativeVersionId
        ) {
          target++;
        }
        if (target === this.entries.length) {
          return null;
        }
        target++;
      }
