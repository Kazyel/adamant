...
Reutilize componentes, tokens e fluxos existentes. Inputs de texto e textareas usam a base global de `ui/shared/styles/inputs.css`. Use `SearchField` para busca, `NumberField` para números e `Form` para formulários com validação visual. Checkboxes e `summary` usam os defaults de `ui/shared/styles/controls.css`. Use `data-tooltip` para dicas visuais, renderizadas por `TooltipLayer`; preserve `title` acessível de iframes e props de títulos de diálogos. Para campos com ícone, reutilize `.input-field`; mantenha nas features apenas regras de layout e evite recriar bordas, fundos ou foco. Os detalhes dos cards dos Kanbans devem abrir em drawers. Preserve navegação por teclado, retorno do foco, rolagem e respeito a movimento reduzido.
